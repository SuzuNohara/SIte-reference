...
export class ConexionesComponent implements OnInit {
  ...
  constructor(private altaServ: AltaServiceService, private automatas: AutomatasService, private http: Http) {
    this.docReference = environment.AUTO_REFERENCE + '/formatos/SITIO_CONECTADO.csv';
    this.validaCR = false;
  ...
  fileChargue(e) {
    ...
    fileReader.onload = (e) => {
      ...
      validacion = this.altaServ.validarArchivoConectado(this.archivo);
  ...
  showInfo(){
    ...
    + environment.AUTO_REFERENCE + '/img/conectado01.png" class="img-responsive"><hr><p>Una vez el archivo sea valido y el ' +
